...
from crypto_scalper.indicators import kdj, macd, rsi
...
class MarketFilterTests(unittest.TestCase):
    ...
    def test_long_filter_rejects_falling_higher_timeframes(self) -> None:
        ...
        falling_30m = _frame("30m", rsi_value=28.0, previous_rsi=31.0, hist=-0.3, previous_hist=-0.2, k=35.0, d=45.0)
        falling_1h = _frame("1h", rsi_value=26.0, previous_rsi=29.0, hist=-0.4, previous_hist=-0.3, k=30.0, d=42.0)
        allowed, reason = market_filter.evaluate(Direction.LONG, [fast, falling_30m, falling_1h])
    ...
    def test_short_filter_rejects_rising_higher_timeframes(self) -> None:
        ...
        rising_30m = _frame("30m", rsi_value=72.0, previous_rsi=68.0, hist=0.3, previous_hist=0.2, k=70.0, d=58.0)
        rising_1h = _frame("1h", rsi_value=74.0, previous_rsi=70.0, hist=0.4, previous_hist=0.3, k=75.0, d=60.0)
        allowed, reason = market_filter.evaluate(Direction.SHORT, [fast, rising_30m, rising_1h])
        self.assertFalse(allowed)
